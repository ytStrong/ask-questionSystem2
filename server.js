...
function checkLogin(req,res,next){
	...
	if (username) {
		...
		//req.xhr 是一个布尔值。 true:表示发送的请求是XHR；false：表示是一个表单请求或是一个普通的网页请求
		//console.log(req.xhr)
		if (req.xhr) {
			//表示发送的是xhr请求，表示用户要自己处理数据
			res.status(200).json({code:0,msg:"登录异常，请重新登录"});
		} else{
			//代表一个表单请求，代表用户不能处理数据，意味着我们帮前端处理
...
app.get("/answer-page",function(req,res){
	res.status(200).render("answer",{title: "回答问题"});
});
// 发送回答问题请求接口
